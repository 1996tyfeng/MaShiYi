...
2. 读取需要修改的目标文件；
3. 根据北京时间（UTC+8）确认记录日期；
4. 用公式 `Day = 记录日期 - 2026-06-25` 计算 Day 编号；
5. 判断是追加还是修正；
6. 对 Markdown 或 CSV 做最小必要修改；
7. 保持文件编码为 UTF-8；
8. 提交前检查表头、日期、Day 编号；
9. 使用清晰 Commit 信息；
10. 回复本次修改内容。
...
当系统时间、AI 本地时间或 UTC 日期与妈妈时间冲突时，以妈妈时间优先。
...
如果 CSV、Diary 或其他文档中的日期与 Day 编号冲突，以日期为准重新计算 Day。
